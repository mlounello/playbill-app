...
function setPostAuthNext(nextPath: string) {
  // Short-lived cookie so the server callback route can read it after OAuth
  // 10 minutes is plenty.
  const maxAgeSeconds = 60 * 10;
  const value = encodeURIComponent(nextPath);
  document.cookie = `pb_next=${value}; Path=/; Max-Age=${maxAgeSeconds}; SameSite=Lax; Secure`;
}
